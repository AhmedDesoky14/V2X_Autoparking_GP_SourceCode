...
WB = 2.85  # rear to front wheel
W = 1.4 #1.86  # width of car
LF = 3.89  # distance from rear to vehicle front end
LB = 1.0  # distance from rear to vehicle back end
MAX_STEER = np.deg2rad(30.0)  # [rad] maximum steering angle
...
def main():
    ...
    plot_car(x, y, yaw)
    for i in range(10):
        x, y, yaw = move(x, y, yaw, .2, pi/4)
        plot_car(x, y, yaw)
        print(x, y, yaw)
    '''
    print(x, y, yaw)
    x, y, yaw = move(x, y, yaw, 1, 1.)
    plot_car(x, y, yaw)
    print(x, y, yaw)
    '''
    plt.show()
